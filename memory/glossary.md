...
| Market tab / page-market | Single nav tab merging Market Overview + Market Signals + Breakout Scanner (roadmap, 2026-06-17). `switchTab('market')` shows `page-market`; a sub-tab bar (`.market-subnav`/`.subtab-btn`) switches between the three `.market-subpage` divs (`subpage-market-overview`/`subpage-market-signals`/`subpage-gapgo`) |
| MARKET_SUBS | Dashboard const array `["market-overview","market-signals","gapgo"]` — the single source of truth for the Market tab's valid sub-ids. Used by `marketSubTab()` (validation), `applyTabFromUrl()` (`SUBS = MARKET_SUBS`, deep-link resolution), and `switchTab()` (redirect guard: any sub-id → parent `market` + sub-tab, so keyboard shortcuts / `#gapgo` / legacy `switchTab('gapgo')` keep working) |
| marketSubTab(subId) | Dashboard fn that selects a Market sub-tab (one of `MARKET_SUBS`): toggles the `.market-subpage` + `.subtab-btn` active state, mirrors the sub-tab id to the URL hash + `localStorage.lastTab` (so legacy `#market-overview`/`#market-signals`/`#gapgo` deep links resolve), lazy-loads Overview (Signals + Breakout manual), and stores `_marketSub` for restore. Cross-link buttons in each sub-tab header call it |
| Breakout sub-tab / subpage-gapgo | Pre-session breakout/gap analysis, formerly the standalone `gapgo` tab, folded into the Market tab as the third sub-tab 2026-06-17. `loadGapGo()` renders Conviction (±7) + Signal /6 (`calcSignalScore`) per card; manual run. Element `subpage-gapgo`, button `msubtab-gapgo` |
...
| nav-section-label / nav grouping | Sidebar tabs are grouped under `.nav-section-label` headers (regrouped 2026-06-17): ⚡ Trade (Signals · Market · Execution) · 💼 Portfolio (Overview · Positions · Allocation · Risk) · 🔬 Analytics (Performance · P&L · Backtest vs Live · Edge · Markov); Command + Settings are ungrouped anchors. Menu-only — tab ids/routing unchanged. Keyboard `TAB_ORDER` (keys 1-9) follows the new visual order |
