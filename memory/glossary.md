...
| `maxSignalSymbols` | `getSettings().limits.maxSignalSymbols` — sets how many of the cap-ranked `TOP30_SYMBOLS` the Market Signals scanner analyses. Default 30, minimum 1, **no upper clamp** (a value above the 30-symbol universe just scans all of them, since `slice` caps at array length). `loadMarketSignals()` uses `SCAN_SYMBOLS = TOP30_SYMBOLS.slice(0, n)`. Does not affect the watchlist Signals tab or Market Overview. |
