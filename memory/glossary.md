...
| `loadConfigFromFile()` / `saveConfigToFile()` | Dashboard config.json I/O. Load = `fetch('./config.json')` then merge into `localStorage` (empty strings don't clobber stored keys; `limits` merged). Save = File System Access API (`showSaveFilePicker`, handle cached in `_configFileHandle`) with a `<a download>` fallback. `saveSettings()` is `async` and awaits the save. |
