...
| getCryptoUniverse() | Dashboard fn that fetches the full tradable crypto universe once (`/v2/assets?asset_class=crypto&status=active`, filtered to tradable `…/USD` pairs), caches it in `_cryptoUniverse`, and orders it as still-tradable `TOP30_SYMBOLS` first then the rest alphabetically. Falls back to `TOP30_SYMBOLS` on failure. Replaces the 30-symbol ceiling on the Market Signals scan so `maxSignalSymbols` can exceed 30 |
