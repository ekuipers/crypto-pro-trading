...
| `loadConfigFromFile()` | Dashboard config.json loader (load-only). `fetch('./config.json')` on page open, then merge into `localStorage` (empty strings don't clobber stored keys; `limits` merged). There is no save-to-file; `saveSettings()` writes `localStorage` only. Edit `config.json` directly to change on-disk defaults. |
