...
| Gap and Go | A large overnight/24h price move that keeps running in the same direction rather than closing the gap. The opposite behaviour is a *fade* — the move reverses and gives the gap back. The Breakout tab rates each symbol's historical tendency between the two |
| ORB (Opening Range Breakout) | Waiting for price to break above or below the range set in the first minutes of a session before entering, instead of guessing the direction of a gap. The Breakout tab recommends it when a symbol's history is a coin flip between gapping and fading |
| VWAP | Volume-Weighted Average Price — the average price paid over a session, weighted by volume traded at each level. Used as a reference line: holding above it is treated as strength, rejection from it as weakness |
| Catalyst quality | How likely a price move is driven by real news rather than noise, judged from the size of the move and the volume behind it. Weak means the move is probably technical noise, so it is not on its own a reason to trade |
| Untranslated by design | Terms the dashboard deliberately leaves in English in every language, because they are identical on every trading platform worldwide: indicator abbreviations (RSI, MACD, ADX, OBV, ATR, VWAP), the action labels BUY / HALF / BEAR / HOLD, and the named patterns Golden cross and Death cross. Everything else the dashboard displays follows the language selector |
