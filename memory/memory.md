...
## 2026-07-18 — Docs: project rename to "CryptoPro Trader" + retention rule (uncommitted, logged retroactively)
...
## 2026-07-19 — Roadmap rescan (via CryptoPro Suite): workflow-rules deduped against Suite's master list; retention pass

- **Retention (rule 11):** today's date (2026-07-19) moved the 2-month retention cutoff to 2026-05-19. Removed `journal/2026-05-18.md` (one day past cutoff) and the two 2026-05-18 walkforward report runs (`reports/walkforward_20260518T115504Z.*`, `reports/walkforward_20260518T145430Z.*`, json+md each). `data/market_research/` had nothing older than the cutoff (oldest is 2026-06-11) — untouched. Confirmed via directory listing: oldest remaining journal is now `2026-05-19.md`, oldest remaining report is `trading-analysis-2026-05-27.md`.
- **Workflow-rules dedup:** CryptoPro Suite's roadmap gained an item — "Add the Workflow rules from project CryptoPro Suite as reference to the sub-project's workflow rules. Remove every duplicate workflow rule from the sub-projects." This project's numbered rules 1, 3, 4, 5, 8, 10 were near-verbatim duplicates of Suite `CLAUDE.md` rules 1 (memory.md changelog), 15 (move completed items to memory), 4 (auto-commit/sync), 5 (update documentation — superseded anyway by the more detailed five-file documentation rule already in this file's Trading Agent Instructions section), 6 (roadmap-scan-triggers-implementation), 19 (never delete `.env`). Removed those six and replaced them with a pointer to Suite's `CLAUDE.md` on GitHub. Kept rules 0 (bugs-before-roadmap priority), 1 ("do not start the local node server" — stricter than and distinct from Suite's "only for testing" rule, and this project has no Node server component anyway), 2 (footer content requirements — more detailed than Suite's baseline: description, creator, last-modified, version), 3 (read memory before analyzing), 4 (self-learning lessons rule), 5 (2-month journal/report retention) — none of these have a Suite equivalent.
- **Verified:** `git status` clean before starting; no application code touched, `CLAUDE.md` diff is rule-list only; retained rules re-numbered 0–5 with no semantic change to their text.
- **Follow-up (same session):** the user directly edited `CLAUDE.md` afterward, trimming the remaining rule list further to just the 2-month retention rule (dropping bugs-priority, self-learning — now genuinely covered by two new rules the user added to Suite's own master list — plus don't-start-local-node-server, footer requirements, and read-memory-first, which had no Suite equivalent). Confirmed intentional with the user before committing rather than restoring the project-specific ones.
...
## 2026-07-19 — Node.js port Phase 2: order execution + evaluation loop (`trade.js` + `run_evaluation.js`)

**Problem:** Continuing the same day's Node.js port (CryptoPro Suite roadmap item 2), the user chose to proceed past scaffolding into "the pieces that actually place orders and run the hourly evaluation cycle." Given the financial blast radius, entered plan mode first: spawned 3 parallel Explore agents to read every relevant Python file in full, then a Plan agent to validate the file list/sequencing before writing any code. The Plan agent caught a real gap: `src/risk.js`'s header claimed the streak throttle was an un-ported ships-OFF extra, but `config.json > risk.streak_throttle_enabled` is `true` live and `run_evaluation.py` calls it unconditionally — fixed as Step 0 before anything else.

**Change (14 new `src/*.js` files + `src/testUtils/fetchStub.js`, all with `.test.js` siblings):**

- **Step 0 — `src/risk.js` fix:** ported `effectiveStopPct`, `consecutiveLossesTail`/`consecutiveWinsTail`, `updateStreakThrottle`, `rollingDrawdownPct` + their config constants (`STREAK_THROTTLE_*`, `TRAIL_MODE`, `CHANDELIER_ATR_MULT`, `ROTATION_ENABLED`, `PARTIAL_TP_ENABLED`/`PARTIAL_TP_FRACTION`, `ENFORCE_BUDGET_ON_OPEN_POSITIONS`); corrected the header/scope-note comment.
- **Shared infra:** `src/symbols.js` (`toSlash`), `src/tz.js` (Amsterdam wall-clock formatter, tested across both 2026 EU DST transitions), `src/env.js` (`.env` loader), `src/apiClient.js` (retry/backoff `fetch` wrapper).
- **`src/positionState.js`** — atomic-write state store (temp file same directory, then rename); tests explicitly cover replacing an existing destination file wholesale and cleanup-on-failure.
- **`src/trade.js`** — full order-placement port including the stop-loss self-rejection clamp fix (ported the exact regression tests from `tests/test_trade_stop_clamp.py`) and case-insensitive stop-order dedup.
- **`src/marketData.js`** — new module (no Python equivalent as a standalone file) extracted to resolve the circular `scout.py`/`rebalance.py`↔`run_evaluation.py` coupling Python tolerates via late binding but ESM can't: bar pagination, `aggregateBarsTo4h`, `fetchAllFills`, `fifoRoundTrips`.
- **`src/scout.js`** — ported in full, not deferred, since `config.json > scout.enabled` is `true` live (an earlier draft plan almost deferred it as "optional").
- **`src/strategyConfig.js`** — the score-threshold/sizing constants that live as bare module constants in `run_evaluation.py` (not `risk.py`), plus `assertNotShipped(flag, value, missingFn)`: throws a clear error if any of the 5 still-genuinely-ships-OFF flags (pyramid, conviction sizing, measured-move, breadth gate, chandelier trail) is ever flipped on before its function is ported, instead of a `ReferenceError` deep in a branch.
- **`src/entrySizing.js`**, **`src/journal.js`**, **`src/reconcile.js`** (includes the Bug #6 relative-tolerance dust fix, ported against the exact `tests/test_reconcile.py` fixtures), **`src/evaluateSymbol.js`** (the full held-long/held-short/flat-entry ladder — every network call *and* indicator function dependency-injectable via `deps`, so all 26 branch tests run with zero HTTP stubbing), **`src/rotation.js`**, **`src/runEvaluation.js`** (orchestration `main()`, same `deps` pattern, `npm run evaluate` CLI entry added to `package.json`).
- `CLAUDE.md`/`README.md`/`memory/glossary.md` updated (this entry + a glossary section for the new port-specific terms: the `deps` pattern, `assertNotShipped`, the two-different-dust-tolerances gotcha).

**Verified:** `npm test` — **280/280 passing**, 0 failures, built up incrementally (never added a file on a red suite). Five regression tests ported/added specifically for known past incidents: the Bug #6 dust-tolerance fix, the stop-loss self-rejection clamp, atomic-write replace-existing-file, SELL-before-BUY ordering stability, and case-insensitive stop-order dedup. One real bug caught during testing (not shipped): a test triggered a live network call because `sessionPenaltyActive` wasn't part of `evaluateSymbol.js`'s injectable `deps` — fixed by adding it, since `strategy.session_filter_enabled` is `true` live and the code path runs on every flat-entry evaluation.

**Not done (deliberately):** `.github/workflows/*.yml` — untouched; the Python engine remains the only thing GitHub Actions executes. `scripts/rebalance.py`/`daily_summary.py`/`metrics.py` — deferred (no trigger / journal-only / backtest tooling). `scripts/stop_watchdog.py` — deferred **with a named caveat** in `CLAUDE.md`: it shares `positions_state.json` with `run_evaluation.js`, so the future cutover can't happen script-by-script for these two. A concrete 3-part "parity verified" checkpoint (deterministic-fixture parity, ≥24-cycle live shadow-run parity, state-file round-trip) is defined in `CLAUDE.md` as the gate before any workflow change or `--execute` against Node. A manual dry-run smoke test against the real paper account (read-only, no orders, but does write to the shared `data/positions_state.json`/`journal/*.md` the Python cron also uses) was flagged as an available next step but not run without the user's explicit go-ahead, given that shared-state write risk.
...
## 2026-07-24 — Roadmap rescan: cadence self-monitor false-firing every run + possible missed GH Actions windows (v2026-07-24.1)

**Finding 1 (bug, fixed): CADENCE WARNING false-firing on every evaluation since 2026-07-20.** `trade.yml`'s evaluate cron was deliberately changed from hourly to every-8h on 2026-07-20 (commit "Change trading bot schedule to every 8 hours") to cut GitHub Actions minutes, but `run_evaluation.py`'s cadence self-monitor (built for Bug #4, 2026-07-10) still hardcoded a 90-minute threshold and an "(expected hourly)" message. Result: every single evaluation journaled since — confirmed in `journal/2026-07-22.md` and `journal/2026-07-23.md`, all 6 entries — carried a false `CADENCE WARNING: ... stops were unchecked in the gap`. This exact drift was already spotted and flagged to the user during the 2026-07-21 shadow-run build session (see v2026-07-21 entry above, "Also noticed, not fixed") but left uncorrected for 3 more days. Asked the user how to resolve it; they chose to change cadence again — **once/day for now** (reason given: no Vercel Pro yet), with a stated plan to move to **every 4h** once Vercel Pro is purchased. Note: this project's own CLAUDE.md documents Vercel Pro as already purchased 2026-07-21 for the `/api/cron/dispatch` Vercel Cron dispatcher — that's a different mechanism from `trade.yml`'s GitHub Actions cron, so the two aren't necessarily in conflict, but flagging the discrepancy in case it's actually the same plan/decision the user has in mind.

**Fix applied (Python + Node kept in parity, per this project's scoring/reconciliation-parity precedent):**
- `.github/workflows/trade.yml`: evaluate cron `2 */8 * * *` → `2 12 * * *` (once/day, 12:02 UTC — arbitrary daily slot between the 03:00 UTC watchdog and 21:23 UTC daily summary); updated the job's `if:` schedule matcher and the stale "Hourly at :23" comment to describe the full throttle history.
- `scripts/run_evaluation.py`: replaced the hardcoded `90`-minute threshold with `CADENCE_EXPECTED_MIN` (24h) / `CADENCE_WARNING_MIN` (25h, +1h slack for GH Actions scheduling jitter) constants; message now says "expected every N min" instead of the stale "expected hourly".
- `src/runEvaluation.js`: identical constants + message change, so the Node port's self-monitor stays in parity ahead of the eventual cutover.
- `src/runEvaluation.test.js`: updated the cadence test's fixture gap from 143 min to 1560 min (26h) so it still exercises the >threshold branch under the new constant.
- `.github/workflows/node-shadow-run.yml`: fixed a stale comment that referenced trade.yml's old `2 */8 * * *` cron for its 15-minute offset rationale — the offset logic itself (own cron `17 */8 * * *`, unchanged) doesn't depend on trade.yml's cadence, so left as-is; only the comment was wrong.
- `CLAUDE.md`'s "Schedule" section rewritten to describe the full cost-throttle history and the parity requirement for future cadence changes.

**Finding 2 (RESOLVED, was a false alarm — see v2026-07-24.2 below):** `data/shadow_run_log.jsonl`'s most recent entry is still 2026-07-23 17:46 UTC as of this rescan (~2026-07-24 07:41 UTC) — the expected ~01:46 UTC follow-up run never landed. Independently, `journal/2026-07-24.md` has zero evaluation entries (last one was 2026-07-23 17:41 UTC) even though `trade.yml`'s separate daily-summary job fired normally at 22:28 UTC the same night — suggesting the evaluate-side schedule may have stopped firing on both workflows around the same time, not a shadow-run-specific issue (unlike the 2026-07-22 credential-only incident). This environment has no `gh` CLI or GitHub API token, and the repo (`github.com/ekuipers/crypto-pro-trading`) returns 404 over plain HTTPS (private), so the Actions run history/logs couldn't be checked here. User is checking the Actions tab directly. **Do not treat this as gate-2 having reset or failed** — it's an observation pending the user's own diagnosis, not a confirmed root cause.

**Other roadmap/bugs sections checked this rescan:** same as 2026-07-23 — Suite's Trader-only GitHub-workflows-removal item is still the only open roadmap item suite-wide, still blocked on gate 2 above (now further delayed by whatever caused finding 2) and gate 4 (Vercel execution-time budget, still unverified in production). Charts/Training/Mobile: no open roadmap or bugs.

---

## 2026-07-24 — Roadmap rescan follow-up: Finding 2 was a stale-local-clone false alarm; gate 2 now PASSES (v2026-07-24.2)

**Root cause of Finding 2, corrected:** the user made the repo public specifically so its Actions run history could be checked. `WebFetch` against `api.github.com/repos/ekuipers/crypto-pro-trading/actions/workflows/{trade,node-shadow-run}.yml/runs` showed both workflows firing normally and successfully every ~7-8h straight through — `trade.yml` run #502 succeeded 2026-07-24T03:31:52Z, `node-shadow-run.yml` run #9 succeeded 2026-07-24T03:43:09Z. Neither had actually stopped. `git fetch CryptoPro-Trader && git log HEAD..CryptoPro-Trader/main` revealed this environment's local checkout was **3 commits behind origin** (`journal: ... 2026-07-24 03:32 UTC`, `shadow-run: ... 2026-07-24 03:43 UTC`, `reports: ... 2026-07-24 06:35 UTC` — all bot-pushed, none pulled locally). The earlier same-day rescan pass read local `data/shadow_run_log.jsonl`/`journal/2026-07-24.md` and concluded runs were missing, when really the runs existed on `origin/main` and just hadn't been fetched into this working copy. `git pull --rebase CryptoPro-Trader main` brought them in cleanly (no conflicts — the local rescan-fix commit and the bot's journal/shadow-run/report commits touch disjoint files).

**Lesson:** before declaring an automated cron/workflow "missing" or "stalled" from evidence in a local git checkout, `git fetch` and diff against the remote tracking branch first — a local clone lagging behind `origin/main` looks identical to a real gap in the data. This is a durable check to run at the *start* of any future roadmap rescan touching cron/workflow health, not just when something looks wrong.

**Gate 2 status, recomputed on the pulled data:** `data/shadow_run_log.jsonl` now has 5 clean automated cycles — 2026-07-22 19:31:39, 2026-07-23 03:44:31 / 10:35:13 / 17:46:13, 2026-07-24 03:43:34 UTC — all `mismatch_count: 0`. Elapsed time from the first automated entry to the most recent: **32h11m55s**, comfortably clearing the ≥24h bar. **Gate 2 now PASSES.** `CLAUDE.md`'s gate-2 paragraph updated from "NOT satisfied" to "PASSED". Gates 1, 2, and 3 are now all passed; **gate 4 (Vercel execution-time budget, needs a real deployment test) is the sole remaining blocker** before the Node cutover can be revisited — still not something to auto-implement, it requires an actual Vercel deployment timing run.
...
## 2026-07-24 — Autopilot/cron-engine coexistence fix (post-cutover-approval investigation scare)

After gate 4 closed and the user said "proceed" with the actual Node cutover, preparing to seed Postgres `trader_state` from `data/positions_state.json` (step 1 of the planned migrate→pause→flip sequence) surfaced a scare: the `trader_state` row already existed (auto-created by the earlier gate-4 dry-run test) and was missing `SOL/USD` — a position the git-committed file still listed as open. Worried this meant my "dry-run" gate-4 test had actually placed a real closing order, checked Alpaca's live order history directly: a genuine SOL/USD sell **did** fill at 2026-07-24T08:10:55Z, but its `client_order_id` was `ap-SOLUSD-1784880655744` — that `ap-` prefix is generated only by `src/js/autopilot.js` (`client_order_id: "ap-" + sym.replace("/", "") + "-" + Date.now()`), the browser-side dashboard Autopilot, never by `src/trade.js` (Node) or Python's `trade.py` (both let Alpaca auto-assign a random UUID). So the sale was the user's own Autopilot, unrelated to my test — confirmed my gate-4 calls placed zero real orders. The "missing" trader_state row was actually **correct**: by the time my dry-run evaluate ran, SOL/USD was already genuinely flat, and reconciliation-from-real-fills correctly excluded it. Seeding from the (now-stale) file would have been a regression, not a fix — the seed script correctly refused to overwrite an existing row, which turned out to be the right call for the wrong reason (there was nothing to migrate; reconciliation self-heals any state that's derivable from broker positions, and the only fields it can't derive — HWM/trailing/partial-TP/breakeven — happened to already be at default/unarmed values in both stores).

**The real, durable finding:** Autopilot is a **third independent trading actor** that was never in the original 4-gate cutover analysis — it has its own state (`localStorage`), places real orders independently of both cron engines, and the 2026-07-21 cutover write-up only ever framed the concurrency risk as "Python vs Node." Raised this to the user before proceeding; they confirmed it's intentional design (Autopilot = fast/reactive while browser is open, cron engine = always-on coverage for gaps like overnight/asleep) and asked for the coexistence to be made to actually work, "the way you think is most feasible."

**Fix:** Autopilot's existing cross-engine merge (roadmap item 10, 2026-07-10 — HWM/partial-TP/entry-time, so a closed browser doesn't miss what the other loop did) only ever read the static `data/positions_state.json` file, which was fine while Python was the only other engine but goes stale/frozen once Python is retired. Added `GET /api/trader-state` (`src/cronRoutes.js`) — unauthenticated (matches the trust level of the static file it supersedes; nothing sensitive in entry prices/HWM), prefers the Postgres `trader_state` row, falls back to `ps.loadState()` (the file) for local dev / pre-cutover. Updated `src/js/autopilot.js` (HWM/partial-TP/entry-time merge) and `src/js/tabs-command.js` (split-HWM Command-tab warning) to try this endpoint first, static file paths kept as fallback. Verified locally: `curl localhost:PORT/api/trader-state` returns the live Postgres state (3 real positions, matching Alpaca's actual `/v2/positions` exactly). All 305 existing tests still pass (no logic in the ported engines touched, only a new read endpoint + two fetch-path lists).

**Lesson:** when investigating a scare on a live trading system, verify against the actual external source of truth (here: Alpaca's own order history, checked by `client_order_id` naming convention) before concluding what happened — don't infer causation from timing coincidence alone, and don't let anxiety about a preceding action skip the verification step that would either confirm or rule it out. Also: a multi-engine coexistence design (Python + Node cron + browser Autopilot) needs its cross-engine merge points re-audited every time a new engine is added or one is retired — the merge mechanism itself doesn't automatically know the set of engines changed.
