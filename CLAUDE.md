...
*Candidates from the 2026-07-07 dashboard effectiveness & strategy-consistency analysis (see `memory/memory.md`). Owner picks which to implement — delete the ones you don't want. Each item is self-contained for a later "rescan roadmap".*

1. **[HIGH · consistency] Autopilot daily-drawdown gate.** Python blocks all new entries once daily drawdown ≥ 3% (`risk.py › daily_drawdown_gate_triggered` + capital-preservation mode in `data/positions_state.json`); the dashboard Autopilot has no equivalent — it keeps opening positions during a portfolio-level slide. Add the same gate to `apCycle()`: snapshot day-open equity (GMT+2 day, `localStorage`), block entries (exits stay active) when equity falls ≥ `daily_drawdown_gate_pct` below it, log a `[BLOCK]` reason, reset at day roll.
2. **[HIGH · effectiveness] Autopilot: price limits off a fresh quote, not the last completed bar.** `apCycle()` uses `res.lastClose` (last *completed* 15-min bar, up to ~15–30 min stale by design of `barsEnd()`) as the "ask" for entry limits (`×1.001`) and exit limits (`×0.995`). In a fast move the limit can be far from the market → entries never fill or exits chase. Fetch the live snapshot quote (`/v1beta3/crypto/us/snapshots` — already used by the ticker strip) at order time and compute the limit band from that, keeping `lastClose` only for scoring.
3. **[HIGH · effectiveness] Autopilot stale-order lifecycle.** Orders are GTC and nothing ever cancels or replaces them except the ⛔ kill switch. An unfilled exit is skipped forever via the `qty_available` dedup ("qty locked"), leaving the position unprotected — Python instead cancel-replaces after `stop_loss_escalation_cycles` (2) with a wider band. Mirror that: track order age per symbol; after 2 cycles cancel-replace unfilled exits with an escalated band, and cancel unfilled *entry* limits after 1 cycle (the signal is stale by then).
4. **[MEDIUM · consistency] Seed Autopilot strategy constants from `config.json`.** `AP_TA_EXIT_SCORE` (−2), `AP_TRAIL_ARM_PCT` (2.5), `AP_TRAIL_PCT` (3), `AP_CASH_RESERVE_PCT` (20) and the swing-low stop params are hardcoded JS consts while Python reads them from `config.json › strategy/risk`. `loadConfigFromFile()` already fetches `./config.json` on load — seed these values from it (same seed/fallback pattern as `limits`) so a config change can't silently fork the two engines.
5. **[MEDIUM · consistency] Align minimum-bars threshold.** Dashboard scores with `bars.length >= 55` (Signals + Autopilot); Python requires `data.min_bars_for_signal` = 60. Align both at 60 and add the check to the Python ↔ Dashboard consistency checklist.
6. **[MEDIUM · addition] Surface scout promotions in the dashboard.** Python merges `data/watchlist_dynamic.json` promotions into every evaluation; the dashboard never reads it, so the Signals tab, Autopilot, and Command view a narrower universe than the bot actually trades. Fetch it like `config.json` (same-folder relative fetch, graceful failure), show promoted symbols as tagged rows in Signals and a Command-tab chip, and optionally include them in the Autopilot scan (respecting TTL).
7. **[MEDIUM · addition] ADX + OBV informational columns.** The 2026-07-07 Python change added ADX(14) + OBV trend as journal-only context; the dashboard has no counterpart, so the hourly agent sees trend-strength/volume-flow data the dashboard user doesn't. Port `adx()`/`obv_trend()` to JS as **display-only** columns in the Signals and Scalping tables (keep the score-parity exemption — do NOT fold into `calcSignalScore`).
8. **[LOW · addition] R:R preview in Signals table + trade modal.** The Decision Checklist requires "R:R ≥ 1:2" but nothing computes it. Show per-symbol: swing-low stop price (`swingLowStop4h` already exists), stop distance %, and implied R:R to the nearest resistance/BB-upper target, in the Signals table and pre-filled trade modal. Display-only first; optionally later gate Autopilot entries on R:R ≥ 2.
9. **[LOW · addition] Correlation-aware entry gate.** The tier budget is static (BTC/ETH = Tier-1, everything else Tier-2) while the Risk tab already computes the live Pearson ρ matrix. Optionally block or half-size an Autopilot entry when ρ > 0.9 (30-day daily log-returns) against any open position — a live-data upgrade to the correlation budget.
10. **[LOW · consistency] Trailing-stop HWM state is split.** Python persists HWM in `data/positions_state.json`; the Autopilot in `localStorage.autopilotHwm`. If both loops manage the same position the two trailing stops diverge (browser closed → dashboard HWM misses highs). Minimum: read `positions_state.json` (same relative-fetch pattern) and use `max(localStorage, file)` as the HWM seed; also show a Command-tab warning when the Python state file shows an active HWM for a symbol the Autopilot is managing.
