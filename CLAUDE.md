...
| 🐦 Socials sub-tab (Command) | *(roadmap 2026-07-09, v2026-07-09.6)* Crypto tweets + stats from curated accounts with **> 0.5M followers**. X/Twitter has no keyless API and blocks CORS, so the tab splits the job: **stats are live** via the keyless **fxtwitter API** (`socFetchStats()` → `api.fxtwitter.com/<handle>`, `Access-Control-Allow-Origin: *` — verified working 2026-07-09; live follower + total-tweet counts, static `followersM` snapshots only as render fallback, marked `*`), while **tweet text is best-effort** via **Nitter-mirror RSS** through the same keyless `api.rss2json.com` bridge as News: `socFetchAccount()` tries `SOC_NITTER_HOSTS` (xcancel.com, nitter.poast.org, nitter.privacyredirect.com, lightbrd.com) in order per account — public mirrors come and go (all four were down at ship time; the tab states this and the stats half still works). `loadSocials(force)` runs timelines + stats in parallel `Promise.allSettled` (dead accounts show a red ✕ chip, never blank the tab). The follower gate is enforced by curation (`SOC_ACCOUNTS`, 14 accounts incl. Elon Musk, Binance, CZ, Coinbase, Vitalik, Saylor, Watcher.Guru, Whale Alert, Cointelegraph, Pompliano, Voorhees, Novogratz). **Retweets skipped** (`RT by` prefix); generalist accounts (`general:true`, e.g. elonmusk) keep only crypto-keyword tweets (`SOC_CRYPTO_RE`). Feed capped at `SOC_MAX_PER_ACCT` (8) per account / `SOC_MAX_ITEMS` (60) total, cached `SOC_CACHE_MS` (10 min; ↻ forces). Items reuse the News renderer family: **T1/T2 badges** via `newsCatalystTier()`, coin chips via `newsDetectCoins()`, **All / ⚡ Key only** filter (`socSetFilter()`). Stats UI: per-account chips (`#socAccts`: @handle · live followers · tweets-in-feed, red ✕ when timeline unreachable) + status line (tweets, reachable timelines, live-stats count, combined reach in M followers, key count). Tweet links rewritten from the Nitter mirror back to `x.com` (`socToXUrl()`). Timestamps GMT+2. Auto-loads on sub-tab select. Analysis-only — never places orders; social flow is a defensive input only. |
