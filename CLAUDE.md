...
## Roadmap
*(empty — all 11 items were implemented 2026-07-10 in v2026-07-10.3; see memory.md. The famous-trader package ships **config-flagged OFF** pending walk-forward validation: `strategy.pyramid_enabled`, `risk.trail_mode="chandelier"`, `strategy.conviction_sizing_enabled`, `strategy.measured_move_enabled`, `strategy.breadth_gate_enabled`, `costs.maker_first_entries`. Active by default: the losing-streak/drawdown throttle, the 5-minute stop watchdog, the session-edge filter (self-guarding on min sample), and the walk-forward baseline machinery (fees 25 bps, `reports/walkforward_latest.json`, dashboard staleness warning). To enable a flagged feature: re-run `scripts/walkforward_evaluate.py` with the candidate config, compare against the baseline, then flip the flag.)*
