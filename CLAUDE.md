...
- `src/*.js` ports order execution + the evaluation loop incl. scout, plus (2026-07-21) `stopWatchdog.js` and `dailySummary.js` — every script the GitHub Actions workflows call now has a Node equivalent (298 tests, `npm test`; `npm run evaluate` dry-run). Live engine is still 100% Python.
- Cutover gated on: (1) frozen-fixture decision parity, (2) ≥24h live shadow-run parity, (3) state-file round-trip Python↔Node, (4) confirm each `/api/cron/*` route finishes inside Vercel's function execution-time budget. `-- --execute` / `CRON_EXECUTE=true` not recommended until all four pass. Details: archive › Node.js port.

## Cron cutover (Suite roadmap, "For Trader only", 2026-07-21 — NOT live yet)
- `src/cronRoutes.js` (`GET/POST /api/cron/evaluate|watchdog|daily-summary`) runs the Node engines as Vercel Cron-triggered serverless functions instead of GitHub Actions — `vercel.json` schedules each once daily, 2h apart (02:00/04:00/06:00 UTC). `GET` (the Vercel Cron contract) requires the `CRON_SECRET` bearer header; `POST` (dashboard "Run now") requires signing in as the single `TRADER_OWNER_UID` account — see `.env.example`. State/journal persist to Postgres (`trader_state`/`trader_journal` tables in `src/db.js`, not `positions_state.json`/`journal/*.md`) since a serverless function has no persistent local disk; `job_runs` is the audit trail + concurrency lock (one `status='running'` row per job, DB-enforced via a partial unique index), `cron_config` is the dashboard enable/disable toggle per job.
- **`CRON_EXECUTE` defaults unset/false — these routes are dry-run only today.** GitHub Actions stays the live engine until the Node.js port's 4-gate cutover checkpoint above passes; this exists so the infrastructure (routes, DB schema, dashboard panel) is built and testable ahead of that decision, not so it goes live automatically. Full analysis: `memory/memory.md` v2026-07-21.3/.4.
- Command tab's new "☁ Scheduled Jobs" panel (owner-only, `/api/cron/status`) shows last-run status per job with a per-job enabled toggle and a manual "Run now" trigger.
...
- **☁ Scheduled Jobs panel (2026-07-21, Suite roadmap — cron cutover):** Command tab, below Autopilot. Owner-only (`TRADER_OWNER_UID`); shows last-run status/time per job (evaluate/watchdog/daily-summary) from `/api/cron/status`, a per-job enabled toggle, and a "Run now" manual trigger. See "Cron cutover" above — dry-run only (`CRON_EXECUTE` unset) until the Node/Python parity checkpoint passes.
