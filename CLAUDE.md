...
## Roadmap
*(added 2026-07-10 — profit-maximization analysis; each item is grounded in a named trading practice. The 5 bugs that blocked items 1–5 were fixed later the same day in v2026-07-10.2 — position state now persists across runs and the evaluation cadence is hourly — so the items are unblocked, but each still needs walk-forward validation before enabling; see memory.md.)*
...
## Bugs
*(empty — all 5 bugs from the 2026-07-10 profit-maximization audit were fixed 2026-07-10 in v2026-07-10.2; see memory.md. Headline: `data/positions_state.json` is now committed by the workflow every run and the partial-TP flag is reconciled from Alpaca fill history so it can never re-fire; the bar fetch follows `next_page_token` (Alpaca caps one response at ~7 days); a corrupt negative `avg_entry_price` is replaced with the FIFO-derived cost basis; the evaluation cron is hourly at :23 with a `CADENCE WARNING` self-check and a real daily P&L summary at 23:21; the correlation budget is aligned at 7 total / 5 per tier.)*
