...
- `src/*.js` ports order execution + the evaluation loop incl. scout (280 tests, `npm test`; `npm run evaluate` dry-run). Live engine is still 100% Python.
- Cutover gated on: (1) frozen-fixture decision parity, (2) ≥24h live shadow-run parity, (3) state-file round-trip Python↔Node — plus porting `stop_watchdog.js` first (shared state). `-- --execute` not recommended until then. Details: archive › Node.js port.
...
- **Every 5 min:** `scripts/stop_watchdog.py --execute` via `watchdog.yml` — open-long exit levels only.
