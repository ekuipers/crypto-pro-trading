...
| 🌐 Market tab (parent) | **Market Overview and Market Signals are merged into one nav tab** (`page-market`) with a sub-tab bar (`.market-subnav` → `.subtab-btn`). `marketSubTab(subId)` toggles the two `.market-subpage` divs (`subpage-market-overview` / `subpage-market-signals`), keeps the "🌐 Market" nav button active, and reflects the precise sub-tab in the URL hash + `localStorage.lastTab` so the old deep links `#market-overview` / `#market-signals` still resolve (handled in `applyTabFromUrl()` via the `SUBS` list). Overview auto-loads on selection (contextual/diagnostic); Signals stays manual (action-oriented). Cross-links: Overview header "View matching signals →" and Signals header "← Back to market context" both call `marketSubTab`. Selection state persists across switches because both sub-pages keep their rendered DOM. `switchTab('market')` restores the last-used sub-tab via `_marketSub`. |
