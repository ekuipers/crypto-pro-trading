...
## Roadmap
*(added 2026-07-10 — profit-maximization analysis; each item is grounded in a named trading practice. Bugs list takes preference per rule 0 — Bugs #1 and #4 must be fixed before roadmap items 1–5 can work at all, since they depend on persistent position state and a reliable cadence.)*

1. **Pyramid into winners in strong trends** *(Livermore "add to a winning position" / Turtle 2N adds / Druckenmiller)* — config-flagged (`strategy.pyramid_enabled`, default false until walk-forward validated): when a long is in a strong 4H trend (ADX ≥ 25) and score still ≥ the full-size gate, add up to 2 tranches at +1R and +2R, each at ½ the initial risk, with the stop trailed to breakeven+ after each add. Symbol cap, cash reserve, and correlation budget still bind. Per position, pyramiding and the +1R partial-TP ladder are mutually exclusive: trend mode (ADX ≥ 25) pyramids and lets the trail do the exit; chop mode (ADX < 25) keeps today's scale-out ladder. Rationale: the current exit set only ever *reduces* winner exposure (partial TP sells 50% at +1R); asymmetric compounding requires the opposite when the trend is confirmed.
2. **ATR-adaptive (Chandelier) trailing stop** *(Turtles' 2N exit)* — replace the fixed 3% trail with `max(trailing_stop_trail_pct, k × ATR(4H) / price)`, k ≈ 2.5, so high-vol alts aren't shaken out by noise and low-vol majors don't give back 3% needlessly. Config: `risk.trail_mode: "fixed"|"chandelier"`, `risk.chandelier_atr_mult`. Walk-forward before enabling; mirror in the dashboard Autopilot (parity checklist item).
3. **Conviction-scaled position sizing** *(Druckenmiller "when you have conviction, bet big" / PTJ)* — scale `risk_per_trade_pct` with signal quality instead of a flat 1%: 0.75% at the half gate (2.5–3.4), 1.0% at the full gate (3.5–4.9), 1.5% at score ≥ 5 with daily+4H aligned. Caps, reserve, and budget unchanged. CLAUDE.md already says "save full size for ≥ 5/6 setups" — this encodes it in sizing.
4. **Losing-streak / drawdown throttle** *(Paul Tudor Jones: trade smallest when trading worst)* — after 3 consecutive losing round-trips (FIFO) or a rolling 7-day equity drawdown ≥ 5%, halve risk-per-trade until 2 consecutive winners or the drawdown recovers to < 2.5%. Anti-martingale; complements the existing 3% daily gate which only looks at one day. Config: `risk.streak_throttle_*`.
5. **Trend-based profit targets + stricter R:R gate** *(PTJ 5:1 asymmetry principle)* — the BB-upper target is a chop-range target and systematically understates trend runs. Add a measured-move target (prior 4H swing-high, or entry + 2× the 4H range height) used when ADX ≥ 25, and walk-forward test raising `min_rr_full` 1.5 → 2.0. Feeds the net-R:R gate and the Signals/Scalping R:R columns.
6. **Maker-first order pricing** *(execution-cost discipline — the cheapest edge available)* — round trip currently costs ~50 bps fees + spread, a large share of expected edge per trade. For non-urgent entries, rest the limit at/just inside the **bid** (maker) instead of within 0.2% of the ask (taker), with a 1-cycle timeout before repricing; keep taker pricing for all exits and stops (urgency ladder). Config: `costs.maker_first_entries`, `costs.maker_fee_bps_per_side`. Update `trade.py` band checks + walk-forward fee assumptions accordingly.
7. **5-minute stop-loss watchdog** *(cut losses fast — universal rule, currently violated by cadence)* — a lightweight scheduled script (`scripts/stop_watchdog.py`) that every 5 minutes checks only open positions against their stop/trail/breakeven levels and fires the existing `trade.py` stop path. No research, no scoring, no new entries. Decouples loss-cutting from the hourly evaluation so a crash between evaluations can't run unprotected for hours (see Bugs #4: real cadence is 5–7 evals/day, not 24).
8. **Monthly walk-forward re-baseline** *(never trade an unvalidated parameter set)* — the newest walk-forward report is 2026-05-14, pre-dating the 2026-06-19 gate loosening and the entire 2026-07-09 economics/ladder package. Re-run `scripts/walkforward_evaluate.py` (fees 25 bps) against the current config, write to `reports/`, update the Backtest vs Live expected metrics, and add a scheduled monthly run + a staleness warning in the dashboard when the baseline is > 45 days old.
9. **Enable the session-edge filter** — built 2026-07-09, ships OFF (`strategy.session_filter_enabled=false`). Turn on once ≥ `session_min_sample` (20) FIFO round-trips exist per GMT+2 hour/weekday bucket; verify with the Edge sub-tab that flagged buckets are genuinely negative-expectancy first.
10. **Portfolio-level breadth/regime gate** *(Wyckoff phases / Weinstein stage analysis applied at book level)* — compute watchlist breadth = % of symbols in a confirmed daily uptrend. Breadth ≥ 60%: full budget deployment allowed. Breadth ≤ 30%: majors-only (Tier-1), halve the max-positions budget, hold the rest in cash. Today regime is only per-symbol; breadth stops the bot from deploying into a market-wide markdown one coin at a time.

## Bugs
*(added 2026-07-10 — found during the profit-maximization audit; priority order. #1 is the single biggest profit leak in the project.)*

1. **P0 — Position state does not persist between evaluation runs** (`data/positions_state.json`). Evidence: `day_open_date` is frozen at **2026-06-11**; AAVE/USD's +1R partial take-profit re-fired on **6 consecutive evaluations** (2026-07-09 15:29 → 2026-07-10 07:46), each selling "50%" of the remainder (6.54 → 0.82 → 0.41 → 0.20 → 0.10 → 0.05 AAVE) instead of firing once — the winner was liquidated into dust with 6× fee drag instead of riding the trail; the state file was last committed 2026-06-18 while journal commits land every run. Silently broken features: partial-TP once-only flag + breakeven stop, trailing-stop HWM persistence (trailing stops effectively never survive a run), stale-exit entry clocks, and the daily-drawdown gate (comparing live equity against 2026-06-11's $95,428). Likely cause: the runner's git sync discards or never commits the local state file (reset/clean before pull). Fix: commit `data/positions_state.json` every run alongside the journal (rule 4), never hard-reset over it, AND make the partial-TP check idempotent against Alpaca order history (if a partial-TP SELL for the symbol exists since entry, treat `partial_tp_done` as true) so a lost flag can never re-fire.
2. **P1 — 4H bar fetch chronically short.** Native 4H fetches return only 43–50 bars (< the 51 required) for BTC/ADA/AAVE at nearly every evaluation, and the 1H fallback **also fails** ("1H fallback failed") — so Signal 6 contributes 0 and swing-low stops silently degrade to the fixed −5% on exactly the symbols carrying the most capital. Investigate the 4H window/limit/pagination in `get_crypto_bars` (`data.bars_4h=120` requested) and why `aggregate_bars_to_4h()`'s 1H source comes back empty.
3. **P1 — Corrupt cost basis in evaluation output.** Journal shows `SOL/USD HOLD 29.5132 @ $-4.4931 (-1842.96%)` — a negative average entry price and a nonsense unrealized P&L%. Likely Alpaca's `avg_entry_price` after repeated partial sells, or a plpc scaling error in `run_evaluation.py`'s position line. Any logic reading that field (stale exit's score context, dashboard portfolio P&L, rule-discipline insights) inherits the garbage. Root-cause and guard (clamp/derive entry from FIFO fills when the API value is non-positive).
4. **P1 — Scheduled cadence not running as documented.** No `Research HH:MM` blocks in any journal since **2026-05-21** (spec: hourly), and only **5–7 evaluations/day** land instead of 24 (spec: every hour at :23) — stops go unchecked for multi-hour overnight gaps (e.g. 22:46 → 04:04). The 23:21 closing journal also produces no P&L/equity summary (the "daily summary" commits contain only an evaluation block). Fix the schedulers, restore the hourly research routine, and add cadence self-monitoring: journal a `CADENCE WARNING` when the previous evaluation is > 90 minutes old.
5. **P2 — Correlation-budget config is internally inconsistent and out of sync with docs.** `risk.max_open_positions=15` is unreachable with `max_positions_per_tier=5` and only 2 Tier-1 symbols (effective ceiling = 7); the CLAUDE.md hard-rules table and dashboard Settings defaults still describe 4 total / 3 per tier. Align config, dashboard defaults, and the hard-rules table (and decide the intended budget: 15 implies raising the per-tier cap or adding tiers).
