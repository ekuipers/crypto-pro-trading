...
| Market Signals tab | On-demand full 6-point confluence scanner for the `TOP30_SYMBOLS`. Same scoring logic as Signals tab. Number of symbols scanned is capped by the **🔭 Signals Analysis › Max Symbols** setting (`maxSignalSymbols`, default 30): `SCAN_SYMBOLS = TOP30_SYMBOLS.slice(0, n)` (top-N by market cap). Shows score distribution and Top Opportunities panel. Scores cached into `_msPrevScores` for cross-tab display. |
