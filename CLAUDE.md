...
- **Hobby-plan constraint, confirmed 2026-07-21 (broke deployment, now fixed):** this project is on **Vercel Hobby**, which only allows once-daily cron schedules — an hourly `vercel.json` cron entry (the first version of this dispatcher) was rejected at deploy time and silently blocked the next deployment from triggering at all. Fixed: `vercel.json`'s own cron is back to once daily (`0 0 * * *`, a redundant safety net) and a new `.github/workflows/cron-dispatch-ping.yml` — a GitHub Actions job that does nothing but `curl` `/api/cron/dispatch` with the `CRON_SECRET` bearer token, hourly, no Python/checkout/git-commit — provides the actual hourly cadence the adjustable schedule needs. **Requires a manual one-time step:** add the same `CRON_SECRET` value as a GitHub repo secret (`gh secret set CRON_SECRET`), not just a Vercel env var, or the pinger 401s every hour.
- **`CRON_EXECUTE` defaults unset/false — these routes are dry-run only today.** GitHub Actions stays the live engine until the Node.js port's 4-gate cutover checkpoint above passes; this exists so the infrastructure (routes, DB schema, dashboard panel) is built and testable ahead of that decision, not so it goes live automatically. Full analysis: `memory/memory.md` v2026-07-21.3/.4/.5/.6.
