...
## Roadmap
1. Merge Performance + P&L + Edge into one "Analytics" tab with sub-tabs. All three are realized-return/expectancy views over the same fills. (Removes the biggest remaining overlap.)
2. Drop standalone Positions, since Portfolio Overview already has a sortable positions table — or make Positions the deep-dive and slim the Overview table.

*(Completed 2026-06-17, v2026-06-17.16 — see `memory/projects/alpaca-trading-agent.md`: navigation regrouped into ⚡ Trade / 💼 Portfolio / 🔬 Analytics sections, and the Breakout Scanner folded into the 🌐 Market tab as a third sub-tab. The remaining two items above were flagged as needing sign-off because they change tab behavior/URLs.)*
...
| 🌐 Market tab (parent) | **Market Overview + Market Signals + Breakout Scanner merged into one nav tab** (`page-market`) with a sub-tab bar (`.market-subnav` → `.subtab-btn`). The valid sub-ids are `MARKET_SUBS = ["market-overview","market-signals","gapgo"]`. `marketSubTab(subId)` toggles the three `.market-subpage` divs (`subpage-market-overview` / `subpage-market-signals` / `subpage-gapgo`), keeps the "🌐 Market" nav button active, and reflects the precise sub-tab in the URL hash + `localStorage.lastTab` so the old deep links `#market-overview` / `#market-signals` / `#gapgo` still resolve (handled in `applyTabFromUrl()` via `SUBS = MARKET_SUBS`, and in `switchTab()` which redirects any of the three sub-ids to the parent — so keyboard shortcuts and legacy `switchTab('gapgo')` calls keep working). Overview auto-loads on selection (contextual/diagnostic); Signals and Breakout stay manual (action-oriented — user clicks ▶). Cross-links: Overview header "View matching signals →", and Signals + Breakout headers "← Back to market context", all call `marketSubTab`. Selection state persists across switches because all sub-pages keep their rendered DOM. `switchTab('market')` restores the last-used sub-tab via `_marketSub`. |
