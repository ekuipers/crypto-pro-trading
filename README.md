...
- **📖 Glossary sub-tab (Command)** *(roadmap 2026-07-18)* — renders `memory/glossary.md` directly in the dashboard: trading terms, acronyms, and dated notes on every indicator, gate, or dashboard feature added, one click away instead of living only in the repo. Fetches the raw file (`../memory/glossary.md`, falling back to a couple of same-directory paths) and renders the small markdown subset the file uses — headers, tables, `**bold**`, `` `code` ``, `---` rules — with a search box that filters table rows/paragraphs by substring match (section headers stay visible). 5-min cache, ↻ Refresh forces a re-read. **Offline fallback (bugfix 2026-07-18):** most browsers block `fetch()` of a local sibling file when this page is opened directly from disk (`file://`) with no page-script workaround — the tab previously dead-ended on a red error. It now falls back to a small built-in reference (curated acronyms + core trading terms) whenever the live fetch fails, with a status line explaining why and inviting a retry, so the tab always shows something useful. Read-only reference — never places orders.
