...
Professional trader decision cockpit with 12 tabs in a **left sidebar navigation** (sticky 210px vertical column beside the content; collapses to a horizontal scroll bar on mobile ≤700px): **Command**, **Performance**, **Risk**, **Positions**, **Execution**, **Signals**, **P&L**, **Backtest vs Live**, **Breakout Scanner**, **Market** (a single tab with **Overview** / **Signals** sub-tabs), **Markov**, **Settings**.

The active tab is stored in the URL hash (e.g. `dashboard_professional.html#signals`), so you can bookmark or link straight to any tab instead of always landing on Command, and a browser refresh restores the last tab you had open. (Driven by `switchTab()` writing the hash + `localStorage.lastTab`, and `applyTabFromUrl()` restoring it on load and on `hashchange`.) The 🌐 Market tab additionally routes its two sub-tabs through the hash (`#market-overview` / `#market-signals`), so those legacy deep links still open the right sub-tab.
...
- **📈 Performance tab** — equity curve, rolling metrics, and a set of KPI tiles: **Total P&L** (FIFO realized P&L from fills — same number as the P&L tab's "Total Realized P&L", `+$X.XX` / `-$X.XX` with colour), Total Return %, average return, annualised volatility, best/worst period, filled orders. P&L tile is first and colour-coded green/red. Period selector: 1M / 3M / 6M / 1Y.
...
- **🔥 Breakout Scanner tab** — on-demand pre-session analysis for all 10 watchlist symbols: catalyst rating, market cap / supply risk, gap-and-go likelihood, 6-month range position, key S/R levels, historical gap behaviour, trade plan (strategy, entry, stop, T1, T2), and risk rating. All computed client-side from 6 months of daily bars + 8 days of hourly bars via the Alpaca crypto data API. Symbols ranked by conviction score. Each card header shows two scores: **Conviction** (gap/breakout-specific, max ±7) and **Signal /6** (the standard 6-point `calcSignalScore()` score — identical to the Signals and Market Signals tabs).
- **🌐 Market tab** — Market Overview and Market Signals are merged into one nav tab with a sub-tab bar (`marketSubTab()`). Overview auto-loads (contextual/diagnostic); Signals stays manual (action-oriented). The active sub-tab is mirrored to the URL hash + `localStorage.lastTab` so the legacy deep links `#market-overview` / `#market-signals` keep working and a refresh restores the exact sub-tab. Cross-links connect the two ("View matching signals →" on Overview, "← Back to market context" on Signals), and selection state persists when you switch because both sub-pages keep their rendered tables.
