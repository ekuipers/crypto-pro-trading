...
**10 open items (added 2026-07-10)** from a profit-maximization audit, each grounded in a named trading practice: pyramiding into winners in strong trends (Livermore/Turtles/Druckenmiller), an ATR-adaptive Chandelier trailing stop (Turtle 2N), conviction-scaled position sizing, a losing-streak/drawdown throttle (Paul Tudor Jones), trend-based profit targets with a stricter R:R gate, maker-first order pricing to cut the ~50 bps round-trip fee drag, a 5-minute stop-loss watchdog, a monthly walk-forward re-baseline, enabling the session-edge filter once sampled, and a portfolio-level breadth/regime gate (Weinstein stage analysis). Full specifications in `CLAUDE.md › Roadmap`.

**5 open bugs (added 2026-07-10)** take preference per the workflow rules — headline: `data/positions_state.json` does not persist between evaluation runs (P0 — it froze on 2026-06-11 and caused the AAVE partial take-profit to re-fire on 6 consecutive evaluations, liquidating the winner into dust), the 4H bar fetch is chronically short with a failing 1H fallback, a corrupt negative cost basis appears in evaluation output, the documented hourly research/evaluation cadence is not actually running (5–7 evals/day instead of 24, no research blocks since 2026-05-21), and the correlation-budget config is internally inconsistent (15 total positions unreachable with 5 per tier). Details in `CLAUDE.md › Bugs`.

Latest shipped fix (2026-07-10, v2026-07-10.1): the **Socials sub-tab RSS bug** ("RSS reader not yet whitelisted!") — xcancel serves that error as a fake HTTP-200 feed which rendered as tweets, and every public Nitter mirror now blocks keyless RSS readers. `socFetchAccount()` now validates the feed title against the account handle and fetches each account's **official Telegram mirror first** (RSS-Bridge TelegramBridge → rss2json; Binance, Watcher.Guru, Whale Alert, Cointelegraph), keeping Nitter as best-effort fallback. See `memory/memory.md`.
