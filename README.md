...
- **📖 Glossary sub-tab (Command)** *(roadmap 2026-07-18)* — renders `memory/glossary.md` directly in the dashboard: trading terms, acronyms, and dated notes on every indicator, gate, or dashboard feature added, one click away instead of living only in the repo. Fetches the raw file (`../memory/glossary.md`, falling back to a couple of same-directory paths) and renders the small markdown subset the file uses — headers, tables, `**bold**`, `` `code` ``, `---` rules — with a search box that filters table rows/paragraphs by substring match (section headers stay visible). 5-min cache, ↻ Refresh forces a re-read. Read-only reference — never places orders.
